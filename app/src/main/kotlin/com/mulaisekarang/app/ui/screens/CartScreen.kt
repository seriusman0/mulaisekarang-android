...
import com.mulaisekarang.app.data.model.VoucherInfo
...
import com.mulaisekarang.app.viewmodel.CartUiState
        ...
        when {
            ...
            preview == null || (preview.items.isEmpty() && preview.issues.isEmpty()) -> EmptyState(
                ...
                Column(modifier = Modifier.padding(16.dp)) {
                    // Voucher input row — always visible when cart is non-empty
                    VoucherInputRow(
                        uiState = uiState,
                        appliedVoucher = preview.voucher,
                        onVoucherChange = { viewModel.onVoucherInputChange(it) },
                        onApply = { viewModel.applyVoucher() },
                        onRemove = { viewModel.removeVoucher() },
                    )

                    // Totals — show subtotal struck through when a voucher is active
                    Row(
                        modifier = Modifier
                            .fillMaxWidth()
                            .padding(top = 12.dp),
                        ...
                        if (preview.voucher != null && preview.discount > 0) {
...
/** Voucher code input field + apply/remove button. */
@Composable
    ...
    Column {
        Row(
            ...
            OutlinedTextField(
                ...
                singleLine = true,
                isError = uiState.voucherError != null,
                enabled = appliedVoucher == null && !uiState.isApplyingVoucher,
            ...
            if (appliedVoucher != null) {
                // Voucher successfully applied — show "Hapus" affordance
                TextButton(
                    onClick = onRemove,
                    modifier = Modifier.testTag("voucher_remove"),
                ) {
                ...
                Button(
                    onClick = onApply,
                    enabled = uiState.voucherInput.isNotBlank() && !uiState.isApplyingVoucher,
                    modifier = Modifier.testTag("voucher_apply"),
                ) {
                    if (uiState.isApplyingVoucher) {
        ...
        // Inline error — tied to the field, not a snackbar
        if (uiState.voucherError != null) {
            Text(
                uiState.voucherError,
                style = MaterialTheme.typography.labelSmall,
                color = MaterialTheme.colorScheme.error,
                modifier = Modifier
                    .padding(start = 4.dp, top = 4.dp)
                    .testTag("voucher_error"),
            )
        }

        // Success label: show the voucher label when applied
        if (appliedVoucher != null) {
            Text(
